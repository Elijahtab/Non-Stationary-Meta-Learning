...
\emph{hurts}; and the dormant-unit fraction of the learner's heads falls
rather than accumulates over the run. \textbf{(3)~The bottleneck is knowledge restoration, not learning
...
\begin{enumerate}
  ...
  functional re-fitting, not a pathology); and the \emph{head} dormant-unit
  fraction \emph{falls} from 0.9 to 0.35 over the run, so plasticity
  loss~\citep{dohare2024plasticity,sokar2023dormant} in the policy/value
  heads is not the operative failure mode at this horizon (a follow-up probe
  finds first-layer \emph{encoder} dormancy does accumulate ---
  \S\ref{sec:ladder}, plasticity probe).
...
dormant-fraction diagnostic directly and find it \emph{falling} in the
actor/critic heads at our horizon --- consistent with the literature's own
scale observations (plasticity loss emerges over thousands of tasks or
billions of frames~\citep{abbas2023plasticity,nikishin2023injection}) ---
though it does accumulate in the first encoder layer, layer-locally and
much earlier than that literature would predict --- and note the
...
never re-accumulates across switches (replicated at the evaluation protocol:
$0.79 \to 0.52$ actor, $0.79 \to 0.61$ critic, median over 8 seeds). A
follow-up pre-registered probe extended the measurement to the encoder and
\emph{corrects the scope} of this claim: dormancy in the \emph{first} conv
layer accumulates monotonically ($0.13 \to 0.48$ at the same threshold,
robust at $\tau{=}0.1$) while the two deeper conv layers stay flat or fall.
Plasticity loss --- the failure mode targeted by reset-style
interventions~\citep{dohare2024plasticity,nikishin2022primacy} --- is
therefore absent in the heads at this horizon, consistent with that
literature's own observation that the effect needs much longer
non-stationary exposure~\citep{abbas2023plasticity,nikishin2023injection},
but present layer-locally at the input layer. A pre-registered
ReDo-style intervention against it failed at the mechanism level:
resetting dormant first-layer channels every 10 updates left end-of-run
dormancy \emph{higher} than untreated ($0.55$ vs $0.48$; re-initialised
channels re-die within the cadence) and moved \composite{} by nothing
($-0.013$, $p{=}0.58$). We therefore read the first-layer accumulation as
converged input-layer sparsity --- one-hot grid observations contain mostly
constant planes --- rather than progressive damage; every arm reported
here, including the oracle ceiling, trained under it.
...
\begin{figure}[t]
  ...
  and both content-selection designs (vermillion) are adjudicated negatives
  --- churn-bound, adaptation-bound, and drift-bound respectively
  (\S\ref{sec:gap-anatomy}).}
  \label{fig:memory}
\end{figure}
...
\begin{enumerate}[leftmargin=1.4em, itemsep=0.15em]
  ...
  \item \textbf{Content-addressed selection fails three ways, each
  identified.} Selecting slots by banked-critic value fit, or by a banked
  world-model reward head used as a regime fingerprint, collapses to the
  control: banked heads are scored through a trunk that has \emph{kept
  training} since banking, so the always-fresh live head wins the error
  comparison and the selector almost never restores (restore rates $2\%$ and
  $16\%$ of fires, with well-calibrated triggers). Fixing that drift ---
  banking each slot's \emph{entire} world model so fingerprints are scored in
  their own frozen feature space --- exposes a third obstruction: by the time
  the trigger fires ($\sim$1 update post-switch), the live reward head has
  already adapted to the new regime's sparse rewards, so the ``no switch
  happened'' hypothesis wins the comparison even at true switches
  ($+0.037$, $p{=}0.27$; flip rate on true-switch fires $0.16$). Three
  selectors, three mechanisms --- trunk drift, verification churn, live-side
  adaptation --- one conclusion: a $2{,}048$-step window measured against a
  fast-adapting reward function leaves no stationary signal for content
  addressing on this instrument.
...
self-supervised trigger\} on this instrument: first-exposure spawn dynamics
plus detection economics, with no knob left --- threshold, precision, lag,
and all three content-selection designs are adjudicated. The learned
\emph{which} would need either oracle identity or an instrument whose regime
signal outlives the learner's own adaptation (regimes that differ in
dynamics, not only reward, are the obvious candidate). We release the bank,
the triggers, the selectors, and every probe so that porting this ladder to
such an instrument is a one-afternoon experiment.
...
and our selection probes do fail --- three designs, three identified
mechanical reasons (representation drift, verification churn, live-side
adaptation), the last of which also caps every faster-trigger design ---
while the \emph{when} half proved learnable at half the ceiling on the first
attempt. Notably, these results also bound
...
The dormant-fraction probe originally covered the actor/critic heads; the
follow-up encoder probe (8 seeds, pre-registered) found first-conv-layer
accumulation ($0.13 \to 0.48$), and the follow-up reset intervention could
neither hold it down nor move \composite{} --- consistent with converged
input sparsity, though a causal cost bound remains open. \composite{} is
one facet of recovery; we report \hit80{} alongside but make no
reliability claims.
