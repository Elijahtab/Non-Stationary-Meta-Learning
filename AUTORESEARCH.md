...
| Editable / immutable surfaces | see manifest (`docs/research-notes` is editable; scorer/benchmark/envs immutable; `ppo.py`/`train.py` opened as optional surface 2026-07-06 for the learning-dynamics family, flag-guarded default-off only; `train_brain.py`/`meta_env.py` opened 2026-07-07 for the bottom-rung/oracle CONTROL conditions D0/O1/O2 — [note 0005](docs/research-notes/0005-untested-controller-bottom-rung-oracle.md), user-approved, flag-guarded default-off only; `envs/regime_wrapper.py`+`make_env.py` master-edited 2026-07-17 for the IP-1 `regime_effect="action_flip"` instrument mode — [note 0015](docs/research-notes/0015-instrument-port-dynamics-regimes.md) decision points approved by user, default `goal_swap` byte-identical to the frozen benchmark, regression-tested) | [config/research_manifest.toml](config/research_manifest.toml) |
...
- [LOOP-0017 — conv1-ReDo rung (branch F)](docs/autoresearch-loops/LOOP-0017-conv1-redo-rung.md): OPEN (2026-07-17, home, $0, user green-light) — LOOP-0016 armed branch F; this rung resets dormant conv1 channels (single disclosed config: interval 10, τ=0.025) ([log 0015](docs/research-log/0015-2026-07-17-conv1-redo-preregistration.md)). **CLOSED same-day: P-F1a FAIL at the mechanism level — resets don't stick (conv1 f8 0.550 WITH resets vs 0.479 untreated; re-init channels re-die within the cadence); composite unmoved (−0.0127, p=0.581); P-F1b not read. Reading: converged input-sparsity attractor, not reset-repairable damage — branch F closes with both edges measured; paper probe paragraph updated** ([note 0014 addendum](docs/research-notes/0014-encoder-dormancy-probe.md))
- [LOOP-0018 — IP-1 action-flip calibration](docs/autoresearch-loops/LOOP-0018-action-flip-calibration.md): OPEN (2026-07-17, home, $0, user approved the master env edit) — the instrument port's go/no-go for ~2 GPU-h: `regime_effect="action_flip"` (odd regimes mirror left/right, reward fixed) ([log 0016](docs/research-log/0016-2026-07-17-action-flip-calibration-preregistration.md)). **CLOSED same-day: C-IP-a + C-IP-b both FAIL — IP-1 dead: flip-control at 0.9532 composite (mirrored policy equally competent → ~no forgetting cost), O2-analog restore HURTS (−0.0373, p=0.034), WM error persistence 0 updates. Port killed at the calibration rung before any ladder spend; `action_flip` stays as default-off infrastructure; IP-2 fallback = fresh user call** ([note 0015 §Calibration outcome](docs/research-notes/0015-instrument-port-dynamics-regimes.md))
